...
export default function CreateListing() {
    ...
    const [imageUploadError, setImageUploadError] = useState<false | string>(false);
    ...
    const handleImageSubmit = () => {
        if (files && files.length > 0 && files.length + formData.imageUrls.length < 7) {
            ...
            Promise.all(promises).then((urls)=>{
                setFormData({...formData, imageUrls: formData.imageUrls.concat(urls as string[])});
                setImageUploadError(false);
                setUploading(false);
            }).catch((error) => {
        ...
        <form className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex flex-col flex-1 gap-4">
                ...
                <div className="flex gap-4">
                    ...
                    type="file" 
                    id="images" 
                    className="p-3 border border-gray-300 rounded w-full" accept="image/*" 
                    multiple/>
